...
function hideFinger() {
  if (typeof bus.finger.cb === 'function') {
    bus.finger.cb()
  }
}
// 卡牌转到正面
...
function addPosBtn() {
  ...
  fingerDom.style.left = currentCardPos.left
  fingerDom.style.top = `calc(${currentCardPos.top} + 0.1rem)`
  fingerDom.style.display = 'inherit'
  bus.finger.cb = function() {
...
window.onload = function () {
  ...
  winkBegin();
  loadImg(
    "http://enbrands-2.oss-cn-shanghai.aliyuncs.com/user/26051fc563ba0c5a6b0999732b5caa13.png",
    ["bg-map"]
  ).then(function (res) {
    // console.log(res)
    // mapImg = res
